...
(async () => {
  ...
  const t0 = Date.now();
  await page.goto(url + (url.includes('?') ? '&' : '?') + 'quality=medium', { timeout: 240000, waitUntil: 'domcontentloaded' });
  await page.waitForFunction(() => document.getElementById('loading')?.classList.contains('hide'), null, { timeout: 240000 }).catch(() => console.log('loading did not finish'));
  console.log('loaded in', Date.now() - t0, 'ms');
  for (const a of args) {
    const [cmd, ...rest] = a.split(':');
    if (cmd === 'wait') await page.waitForTimeout(Number(rest[0]));
    else if (cmd === 'shot') { const f = path.join(OUT, rest[0] + '.png'); await page.screenshot({ path: f, timeout: 240000 }); console.log('shot', f); }
