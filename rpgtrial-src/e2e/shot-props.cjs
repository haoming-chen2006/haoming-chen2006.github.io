...
(async () => {
  ...
  page.on('console', (m) => { if (m.type() === "error" || m.type() === "warning" || m.text().startsWith("[inspect]")) errors.push(m.type() + ': ' + m.text().slice(0, 4000)); });
  const t0 = Date.now();
  ...
  for (const a of args) {
    ...
    else if (cmd === 'shot') { await page.waitForFunction(() => document.getElementById('loading')?.classList.contains('hide'), null, { timeout: 120000 }).catch(() => console.log('reloaded and did not finish')); await page.waitForTimeout(400); const f = path.join(OUT, rest[0] + '.png'); await page.screenshot({ path: f, timeout: 120000 }); console.log('shot', f); }
    ...
    else if (cmd === 'eval') { try { const r = await page.evaluate(rest.join(':')); console.log('eval →', JSON.stringify(r)); } catch (e) { console.log('eval failed:', e.message.split('\n')[0]); } }
  }
