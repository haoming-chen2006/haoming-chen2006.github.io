...
//
// Lives under src/ rather than scripts/ because both lanes read it: the dev
// back end (`scripts/designer/prompt.mjs` re-exports it) and the published
// page, where the agent loop runs in the player's own tab.
//
...
// should advertise.
import { SUPPORTED } from '../compile/index.ts';
import { DEFAULTED_PARAMS, PHASES, PLAYER_REFS, ZONES, type SpecError } from '../spec.ts';
import { VOCABULARY, type Block, type TriggerBlock } from '../vocabulary/index.ts';

const byId = <T extends { id: string }>(blocks: T[]): Map<string, T> => new Map(blocks.map((b) => [b.id, b]));
const TRIGGERS = byId(VOCABULARY.triggers);
const CONDITIONS = byId(VOCABULARY.conditions);
const EFFECTS = byId(VOCABULARY.effects);

const line = (id: string, block: Block | undefined, extra = ''): string => {
...
export function systemPrompt(): string {
  ...
  return [
    ...
    ...SUPPORTED.triggers.map((id) => {
      const t: TriggerBlock | undefined = TRIGGERS.get(id);
...
export function failureMessage({ errors = [], compileError, testLog = [] }: FailureReport): string {
