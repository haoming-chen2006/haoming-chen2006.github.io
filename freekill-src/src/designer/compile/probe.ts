 ...
 * makes.
 *
 * This file is portable. It takes the Lua tree as an argument and knows nothing
 * about where it came from: `headless.ts` builds it off disk for node (the
 * create endpoint, the compiler's tests), and `../browser/probe.ts` fetches it
 * for the published page, where the same probe runs in the player's tab.
 *
...
export interface ProbeOptions {
  /**
   * The Lua tree to boot: the shipped bundle plus `test/lua/**`, where the
   * scripted room lives. In node `headless.ts` builds it off disk; in the
   * browser `browser/probe.ts` fetches the two JSON files the build emits.
   */
  bundle: LuaBundle;
  /**
   ...
   * is what the compiler's own suite and the browser do; the create endpoint
   * writes first and passes nothing, so that what it tests is what it wrote.
   */
  files?: Record<string, string>;
}

/**
 * Boot the engine on `bundle` (plus `files`) and probe the hero. Portable: the
 * only platform-specific thing here is `createLuaVm`, which is the same call in
 * node and in a browser tab.
 */
export const probeHero = async (
  spec: HeroSpec, { bundle, files = {} }: ProbeOptions,
): Promise<HeadlessResult> => {
  const tree: LuaBundle = { ...bundle, ...files };
  ...
  const vm = await createLuaVm(tree, {
    logLevels: new Set(),
    onLog: (level, message) => {
