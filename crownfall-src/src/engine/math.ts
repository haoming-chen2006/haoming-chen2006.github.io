import { datan2, dcos, dexp, dhypot, dsin } from './dmath.ts';
...
export const len = (a: Vec): number => dhypot(a.x, a.y);
export const dist = (a: Vec, b: Vec): number => dhypot(a.x - b.x, a.y - b.y);
export const dist2 = (a: Vec, b: Vec): number => { const dx = a.x - b.x, dy = a.y - b.y; return dx * dx + dy * dy; };
export const norm = (a: Vec): Vec => {
  const l = dhypot(a.x, a.y);
  return l < 1e-9 ? { x: 0, y: 0 } : { x: a.x / l, y: a.y / l };
};
export const fromAngle = (ang: number, mag = 1): Vec => ({ x: dcos(ang) * mag, y: dsin(ang) * mag });
export const angleOf = (a: Vec): number => datan2(a.y, a.x);
...
export const easeOutBack = (t: number): number => { const u = clamp01(t) - 1, s = 1.70158; return 1 + (s + 1) * u * u * u + s * u * u; };
...
export const damp = (rate: number, dt: number): number => 1 - dexp(-rate * dt);
